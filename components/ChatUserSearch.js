...
const ChatUserSearch = ({setPopUp, allchats, blockedchats, pendingchats, thisUser, addChatRequest, socket, onlineUsers}) => {
      ...
      if(message!=="") {
        ...
        if (onlineUsers.find((u) => u.userId == user._id)) {
          socket.emit("createChat", ({
            receiverId:user._id, chat:createdchat}));
        };
      }
      else {
      // If the user is online, tell socket
  ...
  return (
    <div className='chat-popup-container' style={{height:'70vh', width:'20%', padding:'10px', top:'60%'}}>
      <div className='user-search-container'>
        <input placeholder='Search Users' className='chat-menu-input' value={userSearch} onChange={(e)=>setUserSearch(e.target.value)} />
        <div style={{color:'red', fontSize:'25px',marginTop:'-5px', cursor:'pointer', justifySelf:'center'}} onClick={()=>setPopUp(false)}>x</div>
      </div>
