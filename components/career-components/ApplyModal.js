...
export const ApplyModal = ({job,open,closeModal,loadingReq}) => {
    const [uploadText,changeUploadtext]=useState("");


    let requirements = loadingReq == true && job != null &&  job.min_requirements ? job.min_requirements.map((skill, idx)=> {
        return (
    ...
    return (
        <div className="applyModal" style={open?{display:"flex"}:{display:"none"}} onClick={() => {
          closeModal()
          document.body.style.overflow = "auto"
        }}>


        {job != null ? <div className="applyModalContent">

          <div className="applyModalClose" onClick={() => {
            closeModal()
            document.body.style.overflow = "auto"
          }}>
            X
