...
const Saved = () => {
  ...
  const fetchData = () => {
    if (token) {
      axios.get('https://koinstreet-learn-api.herokuapp.com/api/v1/savejob/userjobs',
        {
          headers: {
        ...
        .then(function (response) {
          ...
          const savedJobsData = response.data.data.filter(jobInfo => jobInfo.job_id !== null)
  ...
  const removeSavedJob = (e, jobID) => {
    ...
    if (token) {
      ...
      axios.delete(`https://koinstreet-learn-api.herokuapp.com/api/v1/saveJob/${jobID}`,
        {
          headers: {
            'Authorization': `Bearer ${token}`
          }
  ...
  let savedJobsList = savedJobs.map((job) =>
    <div className="saved-job-stub" key={job._id}>
      ...
      <div className="saved-job-stub-footer">
        <div className="saved-job-stub-postDate">Posted: {new Date(job.job_id.updatedAt).toDateString().substr(3)}</div>
        <a className="saved-job-stub-saveLink" onClick={(e) => removeSavedJob(e, job._id)}>Delete</a>
      </div>
    </div>
  )
  return (
    <div className="savedMain">
      ...
      {loading
        ...
        : (userInfo != null ? savedJobsList : <h1>Please sign in to view your saved jobs</h1>)}
    </div>
  )
}
