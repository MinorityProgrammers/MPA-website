...
const EventsPopup = ({ event, popup, togglePopup, token }) => {
  ...
  const { EventPicture, EventDescription, eventLink, catName, eventName, time, host, Virtual, _id, eventStatus } = event;

  const handleReviewFeedback = (event, feedback) => {
    event.preventDefault();
    try {
      axios.patch(`https://koinstreet-learn-api.herokuapp.com/api/v1/event/${_id}`,
        {
    ...
    <div className={`${styles.wrapper} ${popup ? styles.show : styles.hide}`}>
      <div className={styles.container}>
        ...
        <div className={styles.detailsContainer}>
          <div onClick={togglePopup} className={styles.close}><i className="fas fa-times"></i></div>
