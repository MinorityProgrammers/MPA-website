...
const StartupLeftBar = ({ data }) => {
    ...
    //console.log("left bar", data)
    const { tags, website, location } = data;
    let websiteName = website.replace(/^(?:https?:\/\/)?(?:www\.)?/i, "").split('.')[0]
                ...
                <div className="container p-4">
                    ...
                    <div className="row container d-flex flex-column">
                        <div>
                            ...
                            <a className="left__detail-detail tw-mb-2" href={`${data.website}`} target="_blank">{websiteName}</a>
                            <a target="_blank" href={`${data.twitterLink}`} className="mr-2 ml-2"><i className="fab fa-twitter"></i></a>
                            <a target="_blank" href={`${data.facebookLink}`} className="mr-2 ml-2"><i className="fab fa-linkedin-in"></i></a>
                            <a target="_blank" href={`${data.linkedInLink}`} className="mr-2 ml-2"><i className="fab fa-facebook-f"></i></a>
                        </div>
                        <p className="left__detail-topic">Location</p>
                        <p className="left__detail-detail text-cap tw-mb-2">{city}, {country}</p>
