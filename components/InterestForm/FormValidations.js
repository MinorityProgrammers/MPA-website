...
export const validateField = ({ section_1, section_2, section_3, section_4, section_5, section_7 }, values) => {
  if (section_1) {
...
export const validateStepper = (sections, values, nextStep) => {
  let sectionsToArray = Object.keys(sections);
  let indexOfNextStep = sectionsToArray.indexOf(nextStep);
  for (let i = 0; i < indexOfNextStep; i++) {
    ...
    } else if (sectionsToArray[i] === "section_2" && Object.keys(validateSection_2(values)).length !== 0) {
      return {interest: "Field 2 is required"}
    } else if (sectionsToArray[i] === "section_3" && Object.keys(validateSection_3(values)).length !== 0) {
      return {passion: "Field 3 is required"}
    } else if (sectionsToArray[i] === "section_4" && Object.keys(validateSection_4(values)).length !== 0) {
      return {reasons: "Field 4 is required"}
    } else if (sectionsToArray[i] === "section_5" && Object.keys(validateSection_5(values)).length !== 0) {
      return {support: "Field 5 is required"}
    } else if (sectionsToArray[i] === "section_7" && Object.keys(validateSection_7(values)).length !== 0) {
