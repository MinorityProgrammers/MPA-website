...
let localStorageConsultancyKey = "consultancy-questions";
let localStorageStepKey = "consultancy-step";
function Consultancy({ data, active, clickRegister, setClickRegister }) {
  ...
  function getLocalVariables(key, wrapFxn, defaultValue) {
    let hasKey = window.localStorage.getItem(key);
    console.log(hasKey);
    if (hasKey) {
  ...
  useEffect(() => {
    let lenght_of_questions = Object.values(questions).length;
