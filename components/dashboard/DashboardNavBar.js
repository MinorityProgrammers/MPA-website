...
import {
  faGraduationCap, faHome, faTasks, faRocket, faSuitcase, faCalendarDay, faMapMarkedAlt, faUsers, faColumns, faUser, faHamburger, faWallet,
...
const DashboardNavbar = (props) => {
  ...
  const PopulatedNavBar = () => (
    buttons.map((button) => (
      <div
        ...
        style={{ width: '100%' }}
        className={button.componentName === props.currentView ? 'sidebar-item tw-items-center dashboard-navbar-button-selected' : 'sidebar-item tw-items-center dashboard-navbar-button'}
        onClick={() => { props.setView(button.componentName); }}
      >
  ...
  const MobileSideBar = () => (
    ...
    >
      ...
      >
        <img src="/assets/images/mpicon.svg" style={{ width: '60px', paddingBottom: '80px', paddingTop: '40px' }} />
        <PopulatedNavBar selectedComp={props} />
      </div>
      <div
        style={{
      ...
      >
        <button className="w-100 h-100" onClick={() => { setShowNavbar(!showNavbar); }}>
