...
const SwapIcon = () => (
  <svg width="22" height="22" viewBox="0 0 41 40" fill="none" xmlns="http://www.w3.org/2000/svg">
...
const TransactionCard = ({ info }) => {
  ...
  const TransactionType = [
    {
      type: 'deposit',
      icon: faArrowRight,
      className: 'wallet-deposite',
      style: { color: '#FD05B3', transform: 'rotate(135deg)' },
    },
    {
      type: 'exchange',
      icon: faExchangeAlt,
      className: 'wallet-swap',
      style: { color: '#33D2FF', transform: 'rotate(-45deg)' },
    },
    {
      type: 'withdraw',
      icon: faArrowRight,
  ...
  return (
    <div className="d-flex flex-row justify-content-between align-items-center trans-card">
      ...
      <div className="d-flex flex-row justify-content-around align-items-center wallet-button-rows">

        <div
          className={`${transIcon.className} wallet-dashboard-button`}
          style={{
            width: '40px', height: '40px', marginRight: '10px',
          }}
        >
          {transIcon.type === 'exchange' ? <SwapIcon />
            : <FontAwesomeIcon icon={transIcon.icon} style={transIcon.style} className="fa-1x" />}
        </div>
        ...
        <div className="d-flex flex-column justify-content-start align-items center">
          ...
          <p className="trans-card-text" style={{ color: 'rgb(255, 255, 255)', fontSize: '10px' }}>Metamask Wallet</p>
        </div>
      </div>
