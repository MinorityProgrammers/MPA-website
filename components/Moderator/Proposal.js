...
const Proposal = ({proposal}) => {
    ...
    // Replies ___________________
        useEffect(() => {
           fetch (`https://koinstreet-learn-api.herokuapp.com/api/v1/proposal/${proposal._id}/reply`)
        ...
        useEffect(() => {
         const res = axios.get("https://koinstreet-learn-api.herokuapp.com/api/v1/proposalViews/userProposalView", {
            headers: {
     ...
     // Views _______________
                   ...
                   useEffect(() => {
                    const res = axios.get(`https://koinstreet-learn-api.herokuapp.com/api/v1/upVotes/proposalUpvote/${proposal._id}`, {
                       headers: {
                         'Authorization': `Bearer ${token}`
                       }
                     })
                ...
                // upVotes _______________
                  ...
                  useEffect(() => {
                    const res = axios.get(`https://koinstreet-learn-api.herokuapp.com/api/v1/downVotes/proposalDownvote/${proposal._id}`, {
                       headers: {
                         'Authorization': `Bearer ${token}`
                       }
                     })
  ...
  >
    ...
    <div className="views-replies tw-flex tw-flex-col">
      ...
      <div className="elected ">
        <p className="tw-text-center">
          You <span className="tw-font-bold">Elected</span> this
          proposal on <br />
          <span>              
    ...
    <div className="inc-dec-p tw-self-center">
      <p className="tw-flex tw-flex-col">
               <FaSortUp onClick={() => setVotes(upVotes.data.data.length)} />
                    <strong className="tw--my-3">{votes ?  votes : defaultVotes}</strong>
