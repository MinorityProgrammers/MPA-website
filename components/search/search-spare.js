...
const JobsCard = ({ click, popup, data }) => {
  const {job_title, job_description, location, status, easy_apply, 
    company_name, company_description, diversity_score, 
    type_company, website, total_dollars_given, application_link,
  ...
  return (
    <div onClick={click} className={`${styles.container} ${popup ? styles.popupView : null}`}>
      <section className={`${popup ? styles.header : null}`}>
        ...
        <div className={styles.industry}>{company_name} {popup ? <span onClick={() => { }}>{website}</span> : null}</div>
        <div className={styles.location}>{location} {popup ? <span className={styles.remote}>{remote}</span> : null}</div>
        {
          !popup ?
            <>
            ...
            <>
              ...
              <button>Apply Now {application_link}</button>
            </>
        }
      ...
      <section className={styles.main}>
        ...
        <div className={styles.sect}>
          {
            popup ? <div className={styles.sectTitle}>Responsibilities</div> : null
          }
          <div className={styles.sectText}>{job_description}</div>
          {/* <div className={`${styles.list} ${popup ? styles.expand : null}`}><span className={`${styles.listStyle} ${popup ? styles.disc : styles.circle}`} /> one Lorem ipsum dolor, sit amet consectetur adipisicing elit. </div>
          <div className={`${styles.list} ${popup ? styles.expand : null}`}><span className={`${styles.listStyle} ${popup ? styles.disc : styles.circle}`} /> two Lorem ipsum, dolor sit amet consectetur adipisicing elit. </div>
          <div>more...</div> */}
        </div>

        {
          popup
            ...
            <>
              <div className={styles.sect}>
                <div className={styles.sectTitle}>Requirements</div>
                {
                  min_requirements && min_requirements.map(({skill, years}, idx) => (
                    <div className={`${styles.list} ${popup ? styles.expand : null}`}><span className={`${styles.listStyle} ${styles.disc}`} /> Skill: {skill} {' '} Experience: {years} year(s) </div>
                    ))
                }
              </div>
              ...
              <div className={styles.sect_}>
                <div className={styles.sectTitle_}>Salary: {total_dollars_given}</div>
              </div>
